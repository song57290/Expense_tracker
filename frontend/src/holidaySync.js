...
// 앱은 기기에 동기화된 공휴일 캘린더를 그대로 따르므로 이 표가 굳이 최신이 아니어도
// 무방하다. 새해가 되면 해당 연도 항목을 추가해줘야 한다.
const LUNAR_HOLIDAYS_BY_YEAR = {
  2026: [
    ['2026-02-16', '2026-02-18'], // 설날 연휴
    ['2026-03-02', '2026-03-02'], // 삼일절 대체공휴일
    ['2026-05-24', '2026-05-25'], // 부처님오신날 + 대체공휴일
    ['2026-08-17', '2026-08-17'], // 광복절 대체공휴일
    ['2026-09-24', '2026-09-26'], // 추석 연휴
    ['2026-10-05', '2026-10-05'], // 개천절 대체공휴일
  ],
}
...
function getFallbackHolidayDates(fromDateStr, toDateStr) {
  ...
  for (let y = from.getFullYear(); y <= to.getFullYear(); y++) {
    FIXED_HOLIDAYS.forEach(([m, d]) => {
      const date = new Date(y, m - 1, d)
      if (date >= from && date <= to) dates.add(fmtDateStr(date))
    })
    ;(LUNAR_HOLIDAYS_BY_YEAR[y] || []).forEach(([startStr, endStr]) => {
      const cursor = new Date(`${startStr}T00:00:00`)
      const last = new Date(`${endStr}T00:00:00`)
      while (cursor <= last) {
        if (cursor >= from && cursor <= to) dates.add(fmtDateStr(cursor))
...
// 캘린더 탭에 빨간 글씨로 표시할 공휴일 날짜 집합을 돌려준다.
// 네이티브 앱은 기기에 동기화된 공휴일 캘린더를 읽고, 웹은 하드코딩된 표로 대체한다.
