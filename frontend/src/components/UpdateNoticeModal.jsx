  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        {/* 헤더 */}
        <div style={{ padding: '18px 20px 14px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexShrink: 0, background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border)' }}>
          <div>
        ...
        {/* 편집 폼 (관리자) */}
        {editOpen && (
          <div style={{ padding: '16px 14px', background: 'var(--bg-section)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
              <input value={editForm.version} onChange={e => setEditForm(f => ({ ...f, version: e.target.value }))}
                placeholder="버전 (예: ver 2.22)" style={{ flex: 1, padding: '7px 10px', borderRadius: 8, border: '1.5px solid var(--border-input)', fontSize: '0.84rem', background: 'var(--input-bg)', color: 'var(--text-primary)' }} />
              <input value={editForm.date} onChange={e => setEditForm(f => ({ ...f, date: e.target.value }))}
                placeholder="날짜 (예: 2026년 7월 7일)" style={{ flex: 2, padding: '7px 10px', borderRadius: 8, border: '1.5px solid var(--border-input)', fontSize: '0.84rem', background: 'var(--input-bg)', color: 'var(--text-primary)' }} />
            </div>
            <div style={{ fontSize: '0.72rem', color: 'var(--text-faint)', marginBottom: 4 }}>
              UPDATES 배열 (JSON) &nbsp;
            ...
            <textarea value={editForm.updates} onChange={e => setEditForm(f => ({ ...f, updates: e.target.value }))}
              rows={8} style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1.5px solid var(--border-input)', fontSize: '0.78rem', fontFamily: 'monospace', lineHeight: 1.5, resize: 'vertical', background: 'var(--input-bg)', color: 'var(--text-primary)' }} />
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              ...
              <button onClick={() => setEditOpen(false)}
                style={{ padding: '9px 16px', borderRadius: 8, border: '1.5px solid var(--border-input)', background: 'var(--bg-card)', color: 'var(--text-muted)', fontWeight: 600, fontSize: '0.88rem', cursor: 'pointer' }}>취소</button>
            </div>
          </div>
        )}
          ...
          {displayUpdates.map(sec => (
            <div key={sec.section} style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border)', borderRadius: 14, marginBottom: 10, overflow: 'hidden' }}>
              ...
              {sec.items.map((item, i) => {
                ...
                return (
                  <div key={i} style={{ padding: '11px 14px', display: 'flex', alignItems: 'flex-start', gap: 9, borderBottom: i < sec.items.length - 1 ? '1px solid var(--bg-section)' : 'none' }}>
          ...
          {VERSION_HISTORY.length > 0 && (
            <div style={{ marginTop: 4 }}>
              <button
                ...
                style={{ width: '100%', padding: '9px 14px', background: 'none', border: '1.5px solid var(--border)', borderRadius: 10, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'var(--text-faint)', fontSize: '0.78rem', fontWeight: 600 }}
              >
                <span>이전 버전 기록</span>
